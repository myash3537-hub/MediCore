...
alter table public.medicine_batches
  add column if not exists tablets_per_strip integer not null default 10;

alter table public.medicine_batches
  drop constraint if exists medicine_batches_tablets_per_strip_check;

alter table public.medicine_batches
  add constraint medicine_batches_tablets_per_strip_check check (tablets_per_strip >= 1);
...
begin
  ...
  values (nullif(trim(p_customer_name), ''), coalesce(p_discount_amount, 0), coalesce(p_tax_amount, 0), coalesce(p_payment_method, 'Cash'), p_notes, auth.uid())
  returning id into v_sale_id;

  for v_item in select * from jsonb_array_elements(coalesce(p_items, '[]'::jsonb))
  loop
    select stock_quantity into v_batch_stock
    ...
    values (v_sale_id, (v_item ->> 'medicine_id')::uuid, (v_item ->> 'batch_id')::uuid, (v_item ->> 'quantity')::numeric(12, 4), (v_item ->> 'unit_price')::numeric(12, 2), v_line_total);

    insert into public.stock_movements (medicine_id, batch_id, movement_type, quantity, reference_id, notes, created_by)
    values ((v_item ->> 'medicine_id')::uuid, (v_item ->> 'batch_id')::uuid, 'sale', -1 * (v_item ->> 'quantity')::numeric(12, 4), v_sale_id, 'POS sale', auth.uid());
  end loop;
...
begin
  ...
  loop
    ...
    insert into public.sale_return_items (sale_return_id, sale_item_id, batch_id, quantity, refund_amount)
    values (v_return_id, (v_item ->> 'sale_item_id')::uuid, v_batch_id, (v_item ->> 'quantity')::numeric(12, 4), (v_item ->> 'refund_amount')::numeric(12, 2));
...
begin
  ...
  for v_item in select * from jsonb_array_elements(coalesce(p_items, '[]'::jsonb))
  loop
    insert into public.medicine_batches (medicine_id, supplier_id, batch_number, expiry_date, stock_quantity, purchase_price, selling_price, tablets_per_strip, low_stock_threshold)
    values (
    ...
    values (v_purchase_id, (v_item ->> 'medicine_id')::uuid, v_batch_id, (v_item ->> 'quantity')::numeric(12, 4), (v_item ->> 'purchase_price')::numeric(12, 2), (v_item ->> 'selling_price')::numeric(12, 2), v_line_total);

    insert into public.stock_movements (medicine_id, batch_id, movement_type, quantity, reference_id, notes, created_by)
    values ((v_item ->> 'medicine_id')::uuid, v_batch_id, 'purchase', (v_item ->> 'quantity')::numeric(12, 4), v_purchase_id, concat('Purchase invoice ', coalesce(p_invoice_number, 'manual')), auth.uid());
  end loop;
